...
import { Box, Button, ButtonGroup, Center, Container, EmptyState, Heading, HStack, Icon, Spinner, Table, VStack } from "@chakra-ui/react";
import EditableStatus from "@/components/editable-status";
import toast from "react-hot-toast";
import { FaArrowUpRightFromSquare, FaInbox } from "react-icons/fa6";
    ...
    return (
        <Container maxW="container.lg">
            <HStack mt="8" alignItems="center">
                ...
                onSelected={e => toast.promise(editStatus(String(request?._id), e.value), {loading: 'Atualizando status', error: 'Erro ao atualizar status', success: 'Status atualizado com sucesso'})}
                />
            </HStack>
            <Heading fontSize="small" mb="6">#{request?.code}</Heading>
            <Table.Root stickyHeader size="sm">
                ...
                <Table.Body>
                    {request?.expenses && request.expenses.map((expense, i) => (
